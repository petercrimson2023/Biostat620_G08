...
plot_occupation_time_curve <- function(data, time_column, condition_column, condition_values, colors = c("blue", "red"),user) {
  ...
  data_list <- lapply(condition_values, function(cond) {
    ...
    temp_data = data.frame(c = c_values, st = st_values, condition = as.character(cond))
  ...
  temp_gg = ggplot(temp_combined, aes(x = c, y = st, color = condition)) +
    geom_point(size = 1) +
    geom_line() +
    labs(title = paste(user," Social.ST.min grpuped by ", condition_column), 
