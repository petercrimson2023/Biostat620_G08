---
title: "Plots"
author: "Bulun Te"
date: "2024-02-15"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r, message=FALSE, warning=FALSE, include=FALSE}
# Load the data
library(ggplot2)
library(tidyr)
library(dplyr)
Sys.setenv(LANGUAGE = "en")
library(here)
library(patchwork)
setwd(here())
load("GroupData.RData")
```


# Time Series Plot

```{r}

B_plot = ggplot(data = data_bulun ,aes(x=Date, y=Social.ST.min)) + 
  geom_line() + 
  geom_smooth(method = "gam")+
  labs(title="Bulun's Social Screen Time", x="Date", y="Social ST min")

X_plot = ggplot(data = data_xin ,aes(x=Date, y=Social.ST.min)) +
  geom_line() + 
  geom_smooth(method = "gam")+
  labs(title="Xinwei's Social Screen Time", x="Date", y="Social ST min")+
  theme_minimal()

Z_plot = ggplot(data = data_zhang ,aes(x=Date, y=Social.ST.min)) +
  geom_line() + 
  geom_smooth(method = "gam")+
  labs(title="Zhang's Social Screen Time", x="Date", y="Social ST min")+
  theme_minimal()

B_plot + X_plot + Z_plot + plot_layout(ncol = 3)


```

Time series plots shows that three time series have different trends. Bulun's time series has a increasing trend, Xinwei's time series has a almost constant trend and Zhang's time series has a decreasing trend.


## ACF

```{r}

acf(data_bulun$Social.ST.min, main="Bulun's non-academic activities")
acf(data_xin$Social.ST.min, main="Xinwei's non-academic activities")
acf(data_zhang$Social.ST.min, main="Zhang's non-academic activities")

```


ACF plots shows that three time series almost doesn`t have autocorrelation.


# Occupation time plot

```{r,fig.width=15, fig.height=}

Occu_plot = function(data)
{
  c = seq(0, max(data$Total.ST.min) + 1, max(data$Total.ST.min) / 100)
  st = sapply(c, function(x) {
    return(sum(data$Total.ST.min >= x))
  }) / length(data$Total.ST.min)
  temp = data.frame(c, st)
  
  g1 = ggplot(temp, aes(x = c, y = st)) +
    geom_point(size = 1) +
    geom_line(color="blue")+
    labs(title = "Occupation Time Curve for Total Screen Time", x = "Total Screen Time (min)", y = "Occupation Time Curve") + theme(plot.title = element_text(hjust = 0.5))
  
  # Total social screen time
  
  c = seq(0, max(data$Social.ST.min) + 1, max(data$Social.ST.min) / 100)
  st = sapply(c, function(x) {
    return(sum(data$Social.ST.min >= x))
  }) / length(data$Social.ST.min)
  temp = data.frame(c, st)
  
  g2 = ggplot(temp, aes(x = c, y = st)) +
    geom_point(size = 1) +
    geom_line(color="blue")+
    labs(title = "Occupation Time Curve for Social Screen Time", x = "Social Screen Time (min)", y = "Occupation Time Curve") + theme(plot.title = element_text(hjust = 0.5))
  
  # Proportion
  
  c = seq(0, max(data$proportion) + 1, max(data$proportion) / 100)
  st = sapply(c, function(x) {
    return(sum(data$proportion >= x))
  }) / length(data$proportion)
  temp = data.frame(c, st)
  
  g3 = ggplot(temp, aes(x = c, y = st)) +
    geom_point(size = 1) +
    geom_line(color="blue")+
    labs(title = "Occupation Time Curve for Proportion", x = "Proportion", y = "Occupation Time Curve") + theme(plot.title = element_text(hjust = 0.5))
  
  # Pickups
  
  c = seq(0, max(data$Pickups) + 1, max(data$Pickups) / 100)
  st = sapply(c, function(x) {
    return(sum(data$Pickups >= x))
  }) / length(data$Pickups)
  temp = data.frame(c, st)
  
  g4 = ggplot(temp, aes(x = c, y = st)) +
    geom_point(size = 1) +
    geom_line(color="blue")+
    labs(title = "Occupation Time Curve for Pickups", x = "Pickups", y = "Occupation Time Curve") + theme(plot.title = element_text(hjust = 0.5))
  
  
  # Duration per use
  
  c = seq(0,
          max(data$duration_per_use) + 1,
          max(data$duration_per_use) / 100)
  st = sapply(c, function(x) {
    return(sum(data$duration_per_use >= x))
  }) / length(data$duration_per_use)
  temp = data.frame(c, st)
  
  g5 = ggplot(temp, aes(x = c, y = st)) +
    geom_point(size = 1) +
    geom_line(color="blue")+
    labs(title = "Occupation Time Curve for Duration per Use", x = "Duration per Use (min)", y = "Occupation Time Curve") + theme(plot.title = element_text(hjust = 0.5))
  
  g1 + g2 + g3 + g4 + g5 + plot_layout(ncol = 3)
}


Occu_plot(data_bulun)
Occu_plot(data_xin)
Occu_plot(data_zhang)

```

Significant difference in occupation time curves of Total Screeen time and Social Screen time indicates that the three students have different patterns of using screen time.

```{r}

plot_occupation_time_curve <- function(data, time_column, condition_column, condition_values, colors = c("blue", "red")) {

  if (length(condition_values) != 2 || length(colors) != 2) {
    stop("condition_values and colors must be vectors of length 2.")
  }

  data_list <- lapply(condition_values, function(cond) {
    data_filtered <- data %>%
      filter(!!sym(condition_column) == cond)
    
    c_values = seq(0, max(data_filtered[[time_column]]) + 1, length.out = 101)
    st_values = sapply(c_values, function(x) {
      sum(data_filtered[[time_column]] >= x)
    }) / length(data_filtered[[time_column]])
    temp_data = data.frame(c = c_values, st = st_values, condition = as.character(cond))
    
    return(temp_data)
  })
  
  temp_combined <- do.call(rbind, data_list)
  
  ggplot(temp_combined, aes(x = c, y = st, color = condition)) +
    geom_point(size = 1) +
    geom_line() +
    labs(title = "Occupation Time Curve for Social Screen Time", 
         x = "Social Screen Time (min)", 
         y = "Occupation Time Curve") +
    theme(plot.title = element_text(hjust = 0.5)) +
    scale_color_manual(values = setNames(colors, condition_values))
}

plot_occupation_time_curve(data_bulun, "Social.ST.min", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "Social.ST.min", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "Social.ST.min", "semester", c(1, 0), c("blue", "red"))

```

```{r}

plot_occupation_time_curve(data_bulun, "Social.ST.min", "semester_weekday", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "Social.ST.min", "semester_weekday", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "Social.ST.min", "semester_weekday", c(1, 0), c("blue", "red"))

```



```{r}

plot_occupation_time_curve(data_bulun, "proportion", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "proportion", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "proportion", "semester", c(1, 0), c("blue", "red"))

```

```{r}

plot_occupation_time_curve(data_bulun, "proportion", "semester_weekday", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "proportion", "semester_weekday", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "proportion", "semester_weekday", c(1, 0), c("blue", "red"))

```

```{r}

plot_occupation_time_curve(data_bulun, "Pickups", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "Pickups", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "Pickups", "semester", c(1, 0), c("blue", "red"))

```

```{r}

plot_occupation_time_curve(data_bulun, "duration_per_use", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_xin, "duration_per_use", "semester", c(1, 0), c("blue", "red"))
plot_occupation_time_curve(data_zhang, "duration_per_use", "semester", c(1, 0), c("blue", "red"))

```



```{r}

```
