...
```{r}
model_inter <- lm(Social.ST.min ~ Stay.late + Duration.per.use + Temperature_F + Snow + Weekday + Semester +Steps +Steps:Weekday + Temperature_F:Snow, data = data_merge)
summary(model_inter)
vif(model_inter)

model_lag <-  lm(Social.ST.min ~ Social.ST.min.lag1 + Stay.late + Duration.per.use + Temperature_F + Snow + Weekday + Semester +Steps +Steps:Weekday + Temperature_F:Snow, data = data_merge)
summary(model_lag)
vif(model_lag)
```
